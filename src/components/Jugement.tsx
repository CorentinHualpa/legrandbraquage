...
export function Jugement({
  ...
  return (
    <Feuille className="mt-10 border-t-2 border-dashed border-ligne pt-2">
      ...
      {pivot ? (
        <section className="flex flex-col gap-2 border-t-2 border-encre pt-5">
          ...
          <p className="text-[14px] leading-relaxed text-encre-2">
            ...
            l’inverse. Ce n’est pas un avis, c’est le barème : au SMIC, les
            cotisations patronales tombent à{" "}
            <span className="font-semibold text-encre">3,09 % du brut</span>,
            contre 43,05 % à dix mille euros.
          </p>
          <p className="border-l-[3px] border-bleu bg-papier-3 px-3 py-2.5 text-[14px] leading-relaxed">
            Le salaire net médian du privé est de{" "}
            <span className="font-semibold">2 190 €</span> (INSEE 2024). Le point
            de bascule tombe à trente euros de là :{" "}
            <span className="font-semibold">
          ...
          {!complet ? (
            <p className="text-[13px] text-encre-3 italic">
              Ce seuil est calculé sur le périmètre complet, les quatre lignes
              cochées. Le tien n’en compte que{" "}
              {Object.values(perimetre).filter(Boolean).length} : coche tout pour
              te situer par rapport à lui.
            </p>
          ) : null}
        </section>
      ) : null}
      ...
      <Renvoi>
        Sur le seul périmètre des cotisations salariales, celles qu’on voit sur
        sa fiche de paie,{" "}
        <span className="font-semibold">aucun salarié n’est perdant net</span>, à
        aucun niveau de revenu. Le braquage n’apparaît qu’en ajoutant ce qui est
        prélevé avant la paie et après la paie. Ce qu’on voit ne suffit jamais à
        faire le procès.
      </Renvoi>
    </Feuille>
  );
